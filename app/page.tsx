...
import { supabase, getTutorAverageRating } from "@/lib/supabase";
...
export default function Home() {
	...
	const fetchTutors = async () => {
		try {
			const { data, error } = await supabase
				.from("profiles")
				.select("id, full_name, profile_picture, degree");
			...
			const tutorsWithRatings = await Promise.all(
				data.map(async (tutor) => {
					const { average } = await getTutorAverageRating(tutor.id);
					return {
						id: tutor.id,
						name: tutor.full_name,
						profileImage: tutor.profile_picture || "/default.jpg",
						subjects: tutor.degree || "Materia no especificada",
						rating: average || 0,
					};
				})
			);

			const sortedTutors = tutorsWithRatings.sort((a, b) => b.rating - a.rating);
			setTutors(sortedTutors);
		} catch (err) {
			console.error("Error general en fetchTutors: ", err);
		}
	};
	...
	return (
		<>
			...
			<SponsoredCarousel />
			<TutorCarousel tutors={filteredTutors} title="Tutores mejor calificados" />
