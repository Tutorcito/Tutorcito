...
export default function Home() {
  ...
  const fetchUser = async () => {
    try {
      ...
      if (userError || !user) throw new Error("Error en el fetch del usuario.");
      ...
      if (error || !data) throw new Error("Error en el fetch a Supabase.");
  ...
  const tutorsForSelectedSubject = selectedSubject
    ? tutorsData.filter((tutor) =>
  ...
  return (
    <>
      ...
      {selectedSubject && (
        <TutorCarousel
          tutors={tutorsForSelectedSubject}
          title={`Tutores de ${selectedSubject}`}
        />
      )}

      <ClassCardCarousel
        classes={classesData}
        title="Materias más populares"
